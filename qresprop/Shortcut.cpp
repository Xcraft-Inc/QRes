...
BOOL CShortcut::Load(LPSTR szLink)
{
  ...
  if (m_ppf)
  {
    ...
    if (hr == E_FAIL)
      MessageBox(NULL, "Load failed", "shortcut", 0);
    return SUCCEEDED(hr);
  }
  return FALSE;
}
...
BOOL CShortcut::GetPath(LPSTR szRet, int cbMaxRet)
{
  if (GetTarget(szRet, cbMaxRet))
  {
	int iLen = strlen(szRet);

	szRet[iLen] = ' ';
	iLen++;
	HRESULT hr = m_psl->GetArguments(szRet+iLen, cbMaxRet-iLen);
...
void CShortcut::GetFriendlyName(char *szRet, UINT wMaxRet)
{
  char *ptr;

  *szRet = 0;
  WideCharToMultiByte(CP_ACP, 0, m_wszLink, -1, 
                      szRet, wMaxRet, NULL,NULL);
  ...
  ptr = strrchr(szRet, '\\');
  if (ptr)
    memmove(szRet, ptr+1, strlen(ptr));
}
